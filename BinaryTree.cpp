...
struct BinaryTree // structure for binary tree
{
    ...
    BinaryTree()
    { // default initaliser, sets root to null
        root = NULL;
    }
};
...
{
    if (root == NULL)
    {                       // if root == null (entire tree is empty)
        root = new Node(x); // root = new node with value
    }
    else
    {                       // creates new node as roots left or right child, calls real insert Node function
...
{                        // search function, returns node address
    if (current == NULL) // case, node not in tree
    {
        return NULL; // returns null
    }

    else if (current->data == x) // node found
    {
...
Node *BinaryTree::minNodeSubtree(Node *node) // function for finding min node of a subtree
{
    Node *current = node; // creates temp node

    while (current->left != NULL)
    { // keeps travelling down tree, while next left != NULL
        current = current->left;
    }
    return current; // returns location of min node of a subtree
}

Node *BinaryTree::deleteNode(Node *current, int x) // function for deleting nodes
{
    if (current == NULL) // if deleted node doesn't exist
    {
    ...
    else // node exists and time to delete
    {
    ...
    return current; // return curent node
}
...
int main() // driver to control code
{
